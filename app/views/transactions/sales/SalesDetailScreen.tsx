...
export default function SalesDetailScreen() {
    const { transact_id } = useLocalSearchParams<{ transact_id: string }>();
    ...
    const { buyerList } = useBuyerList(1, 25, (buyerSearchQuery.trim() !== "" ? buyerSearchQuery.trim() : undefined));
    ...
    const [saleTransaction, setSaleTransaction] = useState<Omit<SalesTransaction, 'transact_total_amount'> & { transact_total_amount: string }>({
        transact_id,
        buyer_id: "",
    ...
    const saleDetails = useSaleDetails(transact_id);
    ...
    const totalsMatch = useMemo(() => {
        ...
        return selectedItems.reduce(
            ...
            0
        ).toFixed(2) === saleTransaction.transact_total_amount
    }, [selectedItems, saleTransaction.transact_total_amount]);
    ...
    useFocusEffect(
        useCallback(() => {
            if (saleDetails.data != null) {
                const header = saleDetails.data.header;
                const details = saleDetails.data.details;
                setSaleTransaction({
                    transact_id,
                    buyer_id: header.buyer_id,
                    transact_status: header.transact_status,
                    transact_total_amount: header.transact_total_amount.toFixed(2),
                    transact_address: header.transact_address,
                    transact_date: header.transact_date,
                });
                setSelectedBuyer({
                    buyer_id: header.buyer_id,
                ...
                const items = details.map(item => {
                    const stock = inventory.find(i => i.stock_id === item.stock_id);
                    return {
                        stock_id: item.stock_id,
                        quantity: item.item_quantity.toFixed(2),
        ...
        }, [queryClient, scrollRef, transact_id, inventory, saleDetails])
    )
    ...
    const handlePrint = async () => {
        ...
        await PrintReceipt({
            header: {
                transact_id,
                transact_total_amount: Number.parseFloat(saleTransaction.transact_total_amount)
            },
    ...
    if (transact_id === undefined || (transact_id?.trim() === "")) {
        return (
            <View style={[styles.container, { flex: 1, justifyContent: "center", alignItems: "center" }]}>
                <Text style={styles.text_secondary}>Invalid transact_id parameters!</Text>
                <Link style={[styles.text_secondary, { textDecorationLine: "underline" }]} href={"/views/transactions/sales/SalesListScreen"}>Go back</Link>
            </View>
        );
    }
    ...
    return (
        ...
        >
            <Stack.Screen options={{ title: `Sale Details - ${transact_id}` }} />
            <SafeAreaView
                edges={["bottom"]}
