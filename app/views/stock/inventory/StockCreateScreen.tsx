...
export default function StockCreateScreen() {
    ...
    const handleInsert = () => {
        ...
        createStock.mutateAsync(payload).then((res) => {
            if (!res.stock || res.stock === undefined) {
                Toast.show({
                    type: "error",
                    text1: "Error",
                    text2: "Failed to insert stock!",
                })
                return;
            }
            router.replace({
                pathname: "/views/stock/inventory/StockDetailsScreen",
                params: {stock_id: stockData.stock_id},
            })
        });
    };
